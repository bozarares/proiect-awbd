...
<template>
  <div>
    <!-- Navbar -->
    <nav class="bg-zinc-800 text-white px-4 py-2 flex justify-between items-center shadow-lg fixed top-0 left-0 right-0 z-50">
      ...
      <div v-else class="flex items-center space-x-4">
        <NuxtLink v-if="!userStore.token" href="/login" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded transition duration-300">Login</NuxtLink>
        <NuxtLink v-if="!userStore.token" href="/register" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded transition duration-300">Register</NuxtLink>
      </div>
    </nav>

    <!-- Main Content -->
    <div class="bg-zinc-100 pt-16">
