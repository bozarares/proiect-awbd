<template>
	<div class="py-8 bg-gray-100 min-h-screen px-4 md:px-12">
		<NuxtLink :to="`/teams`" class="text-sm">Go Back</NuxtLink>
		<div class="max-w-7xl mx-auto">
			...
			<div v-else-if="team" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<ul class="list-disc pl-5 mb-6">
						<li v-for="member in team.teamMembers" :key="member.id" class="mb-2">
							<div class="flex justify-between items-center">
								...
								<button
									v-if="
										team.user.id === userStore.user.id &&
										member.user.id !== userStore.user.id
									"
              ...
              v-model="email"
							...
							class="border p-2 flex-grow rounded-l-lg"
						/>
...
const {
	...
	pending,
	error,
} = await useAsyncData(`team-${route.params.id}`, async () => {
...
const deleteMember = async (memberId) => {
	try {
		...
		if (error.value) {
			...
			const updatedTeamMembers = team.value.teamMembers.filter(
