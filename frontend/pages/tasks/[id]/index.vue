			...
			<div v-else-if="task" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div class="mt-4">
						...
						<div class="flex flex-wrap mb-4">
							<span
								...
								class="text-white px-3 py-1 rounded-lg mr-2 mb-2"
							>
								{{ label.name }}
								<button
						...
						<form @submit.prevent="addLabel" class="flex flex-col space-y-4">
							...
							>
								<input
									...
									required
									class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 w-full md:w-auto"
								/>
								<input
									v-model="newLabelColor"
									type="color"
									required
									class="h-10 w-10 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
								/>
							</div>
							<button
								type="submit"
				...
				<div class="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
					...
					<form @submit.prevent="addComment" class="mb-6">
						<div class="mb-4">
							<label for="commentText" class="block text-gray-700 font-semibold mb-2"
								>Add a comment:</label
							>
							<textarea
								v-model="commentText"
					...
					<button
						...
						class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg w-full mt-4"
					>
						Load More
					</button>
				</div>
...
const {
	...
	error: errorComments,
} = await useAsyncData(`comments-task-${taskId}-page-0`, async () => {
	if (userStore.token) {
...
const loadMoreComments = async () => {
	try {
		const { data, error } = await useMyFetch(
			`/comments/task/${taskId}?page=${currentPage.value}&size=5`,
			{
				headers: {
					Authorization: `Bearer ${userStore.token}`,
				},
